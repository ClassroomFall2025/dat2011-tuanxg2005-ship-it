...
class QuanLySinhVien:
    ...
    def nhap_sinh_vien(self):
        while True:
            ...
            if loai_sinh_vien == "it":
                ...
                sv = sinhvienpoly.SinhVienIT(ten_sinh_vien,loai_sinh_vien, java, html, css)
                self.danh_sach.append(sv)
            elif loai_sinh_vien == "biz":
                marketing = float(input("Điểm Marketing: "))
                sales = float(input("Điểm Sales: "))
                sv = sinhvienpoly.SinhVienBiz(ten_sinh_vien,loai_sinh_vien, marketing, sales)
                self.danh_sach.append(sv)
            else:
    ...
    def sap_xep_theo_diem(self):
        ds_sap_xep = sorted(self.danh_sach, key=lambda y:y.get_diem(), reverse=True)
